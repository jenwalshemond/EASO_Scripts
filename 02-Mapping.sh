...
bowtie2-build -f /workdir/jlw395/GCA_028769735.1_RI_Zleu_1.0_genomic.fna WCSPindex

##Then GRSP (grasshopper sparrow)
bowtie2-build -f /workdir/jlw395/GCA_020466415.1_ASM2046641v1_genomic.fna GRSPindex
...
[ -f /workdir/jlw395/align_GRSP/bowtie2Commands.txt ] && rm /workdir/jlw395/align_GRSP/bowtie2Commands.txt
...
for SAMPLEID in ${INDS[@]}; do
  ...
  OUTPUT=/workdir/jlw395/align_GRSP/${SAMPLEID}.bam
  ...
  echo "bowtie2 -p 8 --phred33 --very-sensitive-local -x $REFERENCE -I 149 -X 900 --rg-id $SAMPLEID --rg SM:$SAMPLEID -1 $ONESEQ -2 $TWOSEQ -U $USEQ|samtools view -bS > $OUTPUT" >> /workdir/jlw395/align_GRSP/bowtie2Commands.txt

done

parallel -j 4 < /workdir/jlw395/align_GRSP/bowtie2Commands.txt
...
[ -f /workdir/jlw395/align_GRSP/SambambaSortCommands.txt ] && rm /workdir/jlw395/align_GRSP/SambambaSortCommands.txt

cd /workdir/jlw395/align_GRSP/

for BAM in *.bam; do
  OUT=/workdir/jlw395/align_GRSP/${BAM}_sorted.bam

  #create sort file
  echo "/programs/sambamba-0.7.1/sambamba sort -t 30 -m 45G -o $OUT $BAM" >> /workdir/jlw395/align_GRSP/SambambaSortCommands.txt

done

parallel -j 4 < /workdir/jlw395/align_GRSP/SambambaSortCommands.txt
...
[ -f /workdir/jlw395/align_GRSP/qualimapCommands.txt ] && rm /workdir/jlw395/align_GRSP/qualimapCommands.txt
...
for BAM in *.bam; do
  ...
  PDF=${BAM/%.bam/.pdf}
  # qualimap
  echo "/programs/qualimap_v2.2.1/qualimap bamqc -bam $BAM -outfile $PDF --java-mem-size=128G" >> /workdir/jlw395/align_GRSP/qualimapCommands.txt

done

parallel -j 4 < /workdir/jlw395/align_GRSP/qualimapCommands.txt
